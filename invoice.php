...
$DataInv = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM tb_inv WHERE invoice='$noinv'"));
...
<a href="javascript:history.go(-1)" class="btn btn-danger btn-sm px-3 mr-1">Kembali</a>
...
$no = 1;
$tot_bayar = 0;
$data_laporan = mysqli_query($conn,"SELECT * FROM tb_laporan WHERE invoice='$noinv'");
...
<?php
$i4 = mysqli_fetch_assoc(mysqli_query($conn,"SELECT SUM(subtotal) as isub FROM tb_laporan WHERE invoice='$noinv'"));
?>
...
<div class="d-none pt-5 px-5 print-show">
        <div class="text-center mb-5 pt-2">
            <h2 class="mb-3" style="font-size:60px;"><?php echo $nama_toko ?></h2>
            <h2 class="mb-0"><?php echo $alamat_toko ?></h2>
            <h2 class="mb-4">Telp : <?php echo $telp_toko ?></h2>
        </div>
            <h2 class="mb-1">Invoice : <?php echo $noinv ?></h2>
            <h2 class="mb-1">Tanggal : <?php echo $Datee ?></h2>
            <h2 class="mb-1">Kasir : <?php $Casir = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM tb_inv WHERE invoice='$noinv'"));
        echo ''.$Casir['kasir'].''; ?></h2>
    <div class="row">
        ...
        <?php 
        $no = 1;
        $dataprint = mysqli_query($conn,"SELECT * FROM tb_laporan WHERE invoice='$noinv'");
