...
if(isset($_POST['SimpanEdit'])){
    ...
    $queryuser = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM tb_login WHERE id='$id' and password='$pass'"));

    if($queryuser){ 
    $cekDataUpdate =  mysqli_query($conn, "UPDATE tb_login SET username='$uname',telepon='$telp',alamat='$addr'
        WHERE id='$id'") or die(mysqli_connect_error());
    if($cekDataUpdate){
...
if(isset($_POST['UpdateToko'])){
    $utoko = htmlspecialchars($_POST['nama_toko']);
    $utelp = htmlspecialchars($_POST['telp_toko']);
    $uaddr = htmlspecialchars($_POST['alamat_toko']);
    $udes = htmlspecialchars($_POST['deskripsi_toko']);
    $cekDataUpdate1 =  mysqli_query($conn, "UPDATE tb_toko SET
     nama_toko='$utoko',telp_toko='$utelp',alamat_toko='$uaddr',deskripsi_toko='$udes'") or die(mysqli_connect_error());
    if($cekDataUpdate1){
        echo '<script>window.location="pengaturan.php"</script>';
    } else {
        echo '<script>alert("Gagal Edit Data Toko");history.go(-1);</script>';
    }
}; 

if(isset($_POST['UpdatePass'])){
    $pass1 = $_POST['pswd1'];
    $querypass = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM tb_login
     WHERE id='$id' and password='$pass1'"));
        if($querypass){ 

            $pass2 = $_POST['pswd2'];
            $pass3 = htmlspecialchars($_POST['pswd3']);

            if ($pass2==$pass3){
                $cekPass =  mysqli_query($conn, "UPDATE tb_login SET password='$pass3'
                    WHERE id='$id'") or die(mysqli_connect_error());
                    if($cekPass){
                        echo '<script>alert("Password Berhasil di update");window.location="pengaturan.php"</script>';
                    } else {
        ...
        <div class="bg-white shadow rounded overflow-hidden">
            <div class="px-4 bg-purple" style="border-radius:0.25rem;">
                <div class="media align-items-end profile-header">
                    ...
                    <div class="media-body mb-5 text-white">
                        ...
                        <p class="small mb-4"><?php echo $rule ?></p>
                    </div>
                </div>
            </div>
            ...
            <ul class="nav nav-tabs" role="tablist">
                ...
                <?php 
                    if($_SESSION['role']=='admin'){
                        echo '
                        <li class="nav-item">
                <a class="nav-link" data-toggle="tab" href="#PageToko" style="letter-spacing: 1px;">
                <i class="fa fa-store mr-1"></i> Toko</a>
                </li>';
                    }
                        ?>
            </ul>
            ...
            <form method="post">
                    <div class="row">
                        ...
                        <div class="col-sm-6 col-md-6 col-lg-6 pt-6">
                            <div id="Ada1">
                    ...
                    <div id="PagePassword" class="tab-pane fade">
                        <form method="POST">
                    ...
                    <div id="PageToko" class="tab-pane fade">
                        <form method="POST">
                        <div class="row">
                        <div class="col-sm-6 col-md-6 mb-2">
                            <label>Nama Toko<span class="text-danger">*</span></label>
                            <input name="nama_toko" type="text" class="form-control" value="<?php echo $nama_toko ?>" placeholder="nama toko" required>
                        </div>
                        <div class="col-sm-6 col-md-6 mb-2">
                            <label>Telepon Toko<span class="text-danger">*</span></label>
                            <input name="telp_toko" type="number" class="form-control" value="<?php echo $telp_toko ?>" placeholder="0821xxx" required>
                        </div>
                        <div class="col-sm-6 col-md-6 mb-2">
                            <label>Alamat Toko<span class="text-danger">*</span></label>
                            <input name="alamat_toko" type="text" class="form-control" value="<?php echo $alamat_toko ?>" required>
                        </div>
                        <div class="col-sm-6 col-md-6 mb-2">
                            <label>Deskripsi Toko<span class="text-danger">*</span></label>
                            <input name="deskripsi_toko" type="text" class="form-control" value="<?php echo $deskripsi_toko ?>" required>
                        </div>
                        <div class="col-12 text-right">
                             <button type="submit" name="UpdateToko" class="btn btn-primary px-3">Update</button>
                        </div>
                        </div>
                        </form>
                    </div>
