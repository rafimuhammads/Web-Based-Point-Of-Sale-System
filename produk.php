...
<?php 
    $no = 1;
    $data_barang = mysqli_query($conn,"SELECT * FROM tb_produk ORDER BY idproduk ASC");
...
if(isset($_POST['TambahProduk']))
{
    ...
    $cekkode = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM tb_produk WHERE kode_produk='$kodeproduk'"));
    if($cekkode > 0) {
        echo '<script>alert("Maaf! kode produk sudah ada");history.go(-1);</script>';
    } else {
    $InputProduk = mysqli_query($conn,"INSERT INTO tb_produk (kode_produk,nama_produk,harga_modal,harga_jual)
...
if(isset($_POST['SimpanEdit'])){
    ...
    $CariProduk = mysqli_query($conn,"SELECT * FROM tb_produk WHERE kode_produk='$kodeproduk1' and idproduk!='$idproduk1'");
    $HasilData = mysqli_num_rows($CariProduk);

    if($HasilData > 0){
        echo '<script>alert("Maaf! kode produk sudah ada");history.go(-1);</script>';
    } else{
        $cekDataUpdate =  mysqli_query($conn, "UPDATE tb_produk SET kode_produk='$kodeproduk1',
	...
	if(!empty($_GET['hapus'])){
		...
		$hapus_data = mysqli_query($conn, "DELETE FROM tb_produk WHERE idproduk='$idproduk1'");
        if($hapus_data){
          echo '<script>window.location="produk.php"</script>';
        } else {
