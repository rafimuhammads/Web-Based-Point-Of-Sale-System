...
<!-- isinya -->
<?php
$i1 = mysqli_fetch_assoc(mysqli_query($conn,"SELECT SUM(qty) as totqty FROM tb_laporan"));
$i2 = mysqli_fetch_assoc(mysqli_query($conn,"SELECT SUM(qty*harga_modal) as totdpt FROM tb_laporan"));
$i3 = mysqli_fetch_assoc(mysqli_query($conn,"SELECT SUM(subtotal-qty*harga_modal) as totdpt1 FROM tb_laporan"));
$i4 = mysqli_fetch_assoc(mysqli_query($conn,"SELECT SUM(subtotal) as isub FROM tb_laporan"));
?>
...
<?php 
    $no = 1;
    $data_laporan = mysqli_query($conn,"SELECT * FROM tb_inv WHERE status='selesai' ORDER BY id ASC");
  ...
  <tr>
    ...
    <td><?php                    
  $result1 = mysqli_query($conn,"SELECT SUM(qty) AS count FROM tb_laporan WHERE invoice='$oninv'");
  ...
  if($cekrow > 0){
    ...
    <td>Rp.<?php                    
  $result2 = mysqli_query($conn,"SELECT SUM(subtotal) AS count1 FROM tb_laporan WHERE invoice='$oninv'");
...
if(isset($_POST['Remove'])){
  $nona = $_POST['nona'];
  $hapus_data_Cart_all = mysqli_query($conn, "DELETE FROM tb_laporan WHERE invoice='$nona'");
    $hapus_data_Cart_all1 = mysqli_query($conn, "DELETE FROM tb_inv WHERE invoice='$nona'");
