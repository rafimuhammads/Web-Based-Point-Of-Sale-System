...
<?php $dataselect = mysqli_query($conn, "SELECT * FROM tb_produk");
...
if(isset($_POST['InputCart']))
{
    ...
    $cekDulu = mysqli_query($conn,"SELECT * FROM tb_cart ");
    ...
    if($liat>0){
      $cekbrg = mysqli_query($conn,"SELECT * FROM tb_cart WHERE kode_produk='$Input1' and invoice='$inv_c'");
      ...
      if($liatlg>0){
        ...
        $updateaja = mysqli_query($conn,"UPDATE tb_cart SET qty='$baru', subtotal='$baru1' WHERE invoice='$inv_c' and kode_produk='$Input1'");
        if($updateaja){
      ...
      $tambahdata = mysqli_query($conn,"INSERT INTO tb_cart (invoice,kode_produk,nama_produk,harga,harga_modal,qty,subtotal)
  ...
  $queryStar = mysqli_query($conn, "SELECT max(invoice) as kodeTerbesar FROM tb_inv");
  ...
  $oi = $huruf . date("jnyGi") . sprintf("%02s", $urutan);
    ...
    $bikincart = mysqli_query($conn,"INSERT INTO tb_inv (invoice,pembayaran,kembalian,status,kasir) values('$oi','','','proses','')");
    if($bikincart){
      $tambahuser = mysqli_query($conn,"INSERT INTO tb_cart (invoice,kode_produk,nama_produk,harga,harga_modal,qty,subtotal)
...
$DataInv = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM tb_cart LIMIT 1"));
...
$no = 1;
$tot_bayar = 0;
$data_cart = mysqli_query($conn,"SELECT * FROM tb_cart");
while($d = mysqli_fetch_array($data_cart)){
  ...
  <tr>
    ...
    <td><a class="btn btn-danger btn-xs" href="?hapus=<?php echo $d['id']; ?>">
...
if(!empty($_GET['hapus'])){
  $id = $_GET['hapus'];
  $hapus_data_Cart = mysqli_query($conn, "DELETE FROM tb_cart WHERE id='$id'");
      if($hapus_data_Cart){
          echo '<script>history.go(-1);</script>';
      } else {
...
if(!empty($_GET['hapusAll'])){
  ...
  $hapus_data_Cart_all = mysqli_query($conn, "DELETE FROM tb_cart WHERE invoice='$noinvoicenya'");
  $hapus_data_Cart_all1 = mysqli_query($conn, "DELETE FROM tb_inv WHERE invoice='$noinvoicenya'");
    ...
    $itungtrans = mysqli_query($conn,"SELECT SUM(subtotal) as jumlahtrans FROM tb_cart");
  ...
  <div class="row gy-3 align-items-center row-home">
    ...
    <form method="post">
      ...
      <div class="d-block d-sm-block d-md-none d-lg-none py-1"></div>
       <?php 
       $on = mysqli_query($conn,"SELECT * FROM tb_cart");
...
if(isset($_POST['import']))
{
    ...
    $UpdCart = mysqli_query($conn,"UPDATE tb_inv SET
      pembayaran='$Ipembayaran',kembalian='$Ikembalian',status='selesai',kasir='$username' WHERE invoice='$noinv'") 
     or die (mysqli_connect_error()); 

     $UpdLap = mysqli_query($conn, "INSERT INTO tb_laporan (invoice,kode_produk,nama_produk,harga,harga_modal,qty,subtotal)
     SELECT invoice,kode_produk,nama_produk,harga,harga_modal,qty,subtotal FROM tb_cart") or die (mysqli_connect_error());

    $DelCart = mysqli_query($conn,"DELETE FROM tb_cart") or die (mysqli_connect_error());
